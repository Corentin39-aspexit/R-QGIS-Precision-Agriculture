...
Raster_input.df=data.frame(rasterToPoints(Raster_input, spatial=TRUE))
Field_raw_0=data.frame(coords.x1=Raster_input.df[,2],
                       coords.x2=Raster_input.df[,3],
                       Variable=Raster_input.df[,1])
...
# Small corrections on neighbourhood relationships
...
# Robust variance calculations in neighbourhoods
...
if (nrow(SEED_final)<=1){# if no seed, then no zones 
  ...
  Number_zones=1
} else if (nrow(SEED_final)>1){ # if seeds, zones can be created
  # Expanding the zones with the region growing algorithm
...
# Reconstructing a spatial polygon data frame
